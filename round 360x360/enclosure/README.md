...
* **Option 2 Rear-Loading Screen Architecture (Direct M2 Tab Clamping):**
  ...
  * **Direct M2 Screen Tab Clamping:** 2x blind $\varnothing 1.70\text{mm}$ pilot holes ($3.2\text{mm}$ deep) at $X = \pm 12.20\text{mm}$, $Y = -31.50\text{mm}$ matching the blueprint `2-R1` holes allow screwing the screen directly into the front face from the rear.
  * **Forward DuPont Pin Clearance Relief Trench:** $26.0\text{mm} \times 6.0\text{mm} \times 2.2\text{mm}$ pocket allows solder pins and headers protruding forward from the display PCB to clear completely with $>2.4\text{mm}$ solid front wall thickness remaining.
