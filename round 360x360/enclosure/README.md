...
* **Option 2 Rear-Loading Screen Architecture (Direct M2 Tab Clamping):**
  ...
  * **Direct M2 Screen Tab Clamping with Rising Boss Pillars:** 2x solid $\varnothing 4.40\text{mm}$ cylindrical boss pillars rise $1.80\text{mm}$ from the deepened tab pocket floor ($Z = 4.20\text{mm}$) up to the screen seating plane ($Z = 2.40\text{mm}$) at $X = \pm 12.20\text{mm}$, $Y = -31.50\text{mm}$ matching the blueprint `2-R1` holes. Each pillar contains a blind $\varnothing 1.70\text{mm}$ pilot hole ($3.6\text{mm}$ deep) allowing direct M2 screws from the back of the screen.
  * **Deepened Tab Relief & Complete Gap Closure:** The tab pocket floor is recessed to $Z = 4.20\text{mm}$ with an extra $2.2\text{mm}$ trench over the header ($Y = -34.0\text{mm}$), strictly confined to $Y \le -29.20\text{mm}$. This preserves the $100\%$ continuous circular retaining shelf ($r = 27.0\text{mm} \text{ to } 29.92\text{mm}$) around the viewing aperture, completely closing any visible gap or seam into the enclosure while providing ample room for forward-facing DuPont pins and solders around the rising pillars.
