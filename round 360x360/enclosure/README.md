...
* **Option 2 Rear-Loading Screen Architecture (Direct M2 Tab Clamping):**
  ...
  * **Direct M2 Screen Tab Clamping with Reinforced Boss Pillars:** 2x blind $\varnothing 1.70\text{mm}$ pilot holes ($3.4\text{mm}$ deep) supported by $\varnothing 4.20\text{mm}$ solid boss pillars in the front face at $X = \pm 12.20\text{mm}$, $Y = -31.50\text{mm}$ matching the blueprint `2-R1` holes allow screwing the screen directly into the front face from the rear.
  * **Forward DuPont Pin Clearance Relief Trench:** $21.0\text{mm} \times 3.6\text{mm} \times 2.2\text{mm}$ pocket positioned cleanly over the DuPont connector ($Y = -34.0\text{mm}$), preserving maximum solid plastic around the M2 screw pillars with $>2.4\text{mm}$ solid front wall thickness remaining.
